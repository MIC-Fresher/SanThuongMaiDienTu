...
<c:url var="firstUrl" value="/Supplier/Orders/Search?page=1&${parameterUrl.toString()}"/>
<c:url var="lastUrl" value="/Supplier/Orders/Search?page=${requestScope.listOrders.totalPages}&${parameterUrl.toString()}" />
<c:url var="prevUrl" value="/Supplier/Orders/Search?page=${requestScope.listOrders.current - 1}&${parameterUrl.toString()}" />
<c:url var="nextUrl" value="/Supplier/Orders/Search?page=${requestScope.listOrders.current + 1}&${parameterUrl.toString()}" />
<ul class="pagination">
    <c:choose>
        ...
        <c:forEach var="i" begin="${listOrders.begin}" end="${listOrders.end}">
            <c:url var="pageUrl" value="/Supplier/Orders/Search?page=${i}&${parameterUrl.toString()}" />
            <c:choose>
