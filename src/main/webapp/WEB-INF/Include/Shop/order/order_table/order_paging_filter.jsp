...
<c:url var="firstUrl" value="/Shop/Orders/Filter?page=1&${parameterUrl.toString()}"/>
<c:url var="lastUrl" value="/Shop/Orders/Filter?page=${requestScope.listOrders.totalPages}&${parameterUrl.toString()}" />
<c:url var="prevUrl" value="/Shop/Orders/Filter?page=${requestScope.listOrders.current - 1}&${parameterUrl.toString()}" />
<c:url var="nextUrl" value="/Shop/Orders/Filter?page=${requestScope.listOrders.current + 1}&${parameterUrl.toString()}" />
<ul class="pagination">
    <c:choose>
        ...
        <c:forEach var="i" begin="${listOrders.begin}" end="${listOrders.end}">
            <c:url var="pageUrl" value="/Shop/Orders/Filter?page=${i}&${parameterUrl.toString()}" />
            <c:choose>
